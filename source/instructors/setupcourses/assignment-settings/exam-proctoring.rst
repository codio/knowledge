...
- **Restrict IP Addresses** - You must enable **Single Login** (see above) first to use this. Once this is enabled you can enter in the allowable **Public** IP ranges. Each IP address should be on a separate line and you can specify a subnet as depicted below. Students will only be able to open the assignment if their IP matches the specification.
